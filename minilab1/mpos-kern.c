...
//my code
static pid_t do_newthread(process_t *current, int start_function);
...
//my code:
	...
	/*case INT_SYS_KILL:
	{
		...
		if(proc_array[p].p_state != P_ZOMBIE)
		{
			...
			if(proc_array[p].p_wait != 0)//which means someone is blocked since p is running
			{
				pid_t wait = proc_array[p].p_wait ;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	dest_stack_top = PROC1_STACK_ADDR + ((int)(dest->p_pid))*PROC_STACK_SIZE;
