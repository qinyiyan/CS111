...
static int osprd_close_last(struct inode *inode, struct file *filp)
{
	if (filp) {
		...
		{	
			...
			{
				d->number_read_lock--;
				int clearReturn = clear_current_reader(d->pid_list_head, d->pid_list_tail);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		else
		{
			...
			{
				d->number_read_lock --;
				if (clear_current_reader (d->pid_list_head, d->pid_list_tail) == -ENOTTY)
					return -ENOTTY;
			}
			filp->f_flags &= ~F_OSPRD_LOCKED; 
			osp_spin_unlock (&d->mutex);
