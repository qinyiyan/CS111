...
//Exer.7: Lottery scheduling: 
#define RUNCOUNT 320
#define LOTTERYSIZE RUNCOUNT*(NPROCS-1)
pid_t lottery_array[LOTTERYSIZE];
static unsigned long int next = 1;
void give_lottery_number (pid_t * lottery_array);		//assign each process lottery numbers
int rand_lottery_num( void ) ;		//generate random number as lottery
...
start(void)
{
	...
	//Exer.7: lottery scheduling: 
	give_lottery_number( lottery_array );
	...
	//    7 = any algorithm that you may implement for exercise 7: lottery algorithm
	//~~~~~~~~choose an algorithm here~~~~~~~~~~~~`//
	scheduling_algorithm =0;
	//lock = 0;
	//initialize the lock equals 0 which means do not get the lock
	// Switch to the first process.
	proc_array[1].p_run_times ++;
	run(&proc_array[1]);
...
void give_lottery_number (pid_t * lottery_array)
{
	...
	//Generating random number: 	reference: C library: #include "random.h"
