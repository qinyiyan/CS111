...
@tests = (
    ...
    # 18: hardlink and write to hardlink
  ...
  # 20: symlinks
  [ 'ln -s test/symlink.txt thelink ; echo "World" >> test/symlink.txt ; diff test/symlink.txt thelink && echo Same contents ',
    'Same contents'
  ],
  # 21 remove symlink
  [' rm thelink; cat test/symlink.txt',
    'World'
  ],
  # 22: conditional symlinks as root
  [ 'echo "not root">test/notroot; echo "root">test/root; ln -s root?/tmp/cs111/lab3-qinyi/test/root:/tmp/cs111/lab3-qinyi/test/notroot test/amiroot; cat test/amiroot',
    'root'
  ],
  # 23: conditional symlinks as non-root
  [ 'su user -c "cat test/amiroot ; rm test/amiroot test/root test/notroot"',
    'not root'
  ],
