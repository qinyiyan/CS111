...
ospfs_follow_link(struct dentry *dentry, struct nameidata *nd)
{
	...
	//	eprintk("string is %s", oi->oi_symlink);
		//eprintk("second is %s", oi->oi_symlink+pivot+1);
		...
		char* pivot = strchr (oi->oi_symlink, ':');
		//eprintk ("pivot is %s", pivot);
		//eprintk ("address diff = %d", pivot-(oi->oi_symlink+5+1));
		if (current->uid == 0)
		{
			//eprintk("root~");
		//	eprintk("point to %s", oi->oi_symlink+5);
			char* real_add = kmalloc(sizeof(char), GFP_KERNEL);
			memcpy (real_add, (oi->oi_symlink+5), (size_t)(pivot-(oi->oi_symlink+5)));
			//eprintk("real address is %s", real_add);
			nd_set_link(nd, real_add);
		}
		else 	//not root
		{
			//eprintk("notroot");
			//eprintk("pivot+1 is %s", pivot+1);
			nd_set_link(nd, pivot+1);
		}
	return (void*)0;
}
